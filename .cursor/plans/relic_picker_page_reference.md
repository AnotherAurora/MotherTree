...
**Related:** `.cursor/plans/simulator_calculation_pipeline_reference.md` (calculation engine), `.cursor/plans/relic_astral_reign_research_9f2c4b71.plan.md` (relic origins).
...
2. Run the full Path Carver calculation (see `simulator_calculation_pipeline_reference.md` §1; not a shortcut) and show **Total Damage**.
...
Relic rows are a dedicated `sourceKind: "relic"` and behave like realm rows (owner buckets in `simulator_calculation_pipeline_reference.md` §3.5; Layer A gate and interaction immunity in §4 / §6.2 / §10):
...
Reference values at L=50, P=50, no HSR: Arcana Archive 182, Arcana Relic 364, Blessed Blood 74, Deceased's Chrono 530, Neurotoxin 1212, Nettle Vest 606, Uncanny Salve 909, Voyager's Parasol 147. The research notes `.cursor/plans/relic_astral_reign_research_9f2c4b71.plan.md` list all origins.
...
- `totalsOnly` threads `collectSteps: false` into the engine, which skips all debug `steps` object construction (values unchanged; see the `steps` field in `simulator_calculation_pipeline_reference.md` §1). Default remains `true` for Review Tags.
