import type { EnumName, TableName } from "@/lib/database.types";
import { AWAKENER_ENLIGHTENMENT_OPTIONS } from "@/lib/enlightenment-options";
import { formatLabel } from "@/lib/utils";

export type FieldType =
  | "id"
  | "text"
  | "dotSeparated"
  | "number"
  | "boolean"
  | "textarea"
  | "enum"
  | "foreignKey"
  | "timestamp";

export type ForeignKeyRef = {
  table: TableName;
  displayColumn: string;
  /** Composite label for complex parent rows */
  labelKind?: "manifestation";
  /** Filter options to rows where column equals the named form field value */
  filterBy?: { column: string; formField: string };
};

export type FieldConfig = {
  name: string;
  label: string;
  type: FieldType;
  enumName?: EnumName;
  foreignKey?: ForeignKeyRef;
  /** Hide from create/edit forms */
  formHidden?: boolean;
  /** Hide from list table */
  listHidden?: boolean;
  /** Database generates on insert (omit from create forms) */
  autoGenerated?: boolean;
  /** Populated at read time, not stored on the table */
  computed?: boolean;
  required?: boolean;
  defaultValue?: string | number | boolean | null;
  /** Allow click-to-edit directly in the list table */
  inlineEditable?: boolean;
  /** Form layout width; consecutive "half" fields render side-by-side */
  formWidth?: "full" | "half";
  /** When set on a number field, render a labeled select instead of a free number input */
  numberSelectOptions?: readonly { value: number; label: string }[];
};

export type ChildTableConfig = {
  table: TableName;
  label: string;
  /** Parent column whose value is copied into this FK on child rows */
  parentKeyColumn: string;
  /** Child FK column that receives the parent key */
  foreignKeyColumn: string;
  fields: FieldConfig[];
};

export type ListSortDirection = "asc" | "desc";

export type ListSortState = {
  field: string | null;
  direction: ListSortDirection;
};

export type TableConfig = {
  name: TableName;
  label: string;
  description: string;
  /** Column used when this table appears in FK dropdowns */
  displayColumn: string;
  softDelete: boolean;
  /** Order in sidebar (lower = higher) */
  order: number;
  /** Omit from sidebar navigation (managed via a parent table form) */
  sidebarHidden?: boolean;
  /** Child rows edited inline on this table's create/edit form */
  childTables?: ChildTableConfig[];
  uniqueConstraints?: {
    fields: string[];
    ignoreDeleted?: boolean;
    message?: string;
  }[];
  /** Optional alternate list layouts (default: table only) */
  listViews?: {
    tree?: { pathField: string };
  };
  /** Default column sort when the table list first loads */
  defaultListSort?: {
    field: string;
    direction: ListSortDirection;
  };
  fields: FieldConfig[];
};

const timestampFields = (includeDeleted = true): FieldConfig[] => [
  {
    name: "created_at",
    label: "Created At",
    type: "timestamp",
    formHidden: true,
    listHidden: true,
  },
  {
    name: "updated_at",
    label: "Updated At",
    type: "timestamp",
    formHidden: true,
    listHidden: true,
  },
  ...(includeDeleted
    ? [
        {
          name: "deleted_at",
          label: "Deleted At",
          type: "timestamp" as const,
          formHidden: true,
          listHidden: true,
        },
      ]
    : []),
];

export const TABLE_CONFIGS: TableConfig[] = [
  {
    name: "tag",
    label: "Tags",
    description: "Core tag definitions used across interactions and demands.",
    displayColumn: "tag_name",
    softDelete: true,
    order: 1,
    listViews: {
      tree: { pathField: "tag_name" },
    },
    fields: [
      {
        name: "id",
        label: "ID",
        type: "id",
        autoGenerated: true,
        formHidden: true,
      },
      {
        name: "tag_name",
        label: "Tag Name",
        type: "dotSeparated",
        required: true,
      },
      {
        name: "layer",
        label: "Layer",
        type: "enum",
        enumName: "layer",
        inlineEditable: true,
      },
      {
        name: "is_percent",
        label: "Is Percent",
        type: "boolean",
        defaultValue: false,
        inlineEditable: true,
      },
      {
        name: "is_additive",
        label: "Is Additive",
        type: "boolean",
        defaultValue: true,
        inlineEditable: true,
      },
      {
        name: "is_searchable",
        label: "Available in Search",
        type: "boolean",
        defaultValue: false,
        inlineEditable: true,
      },
      {
        name: "is_search_simulated",
        label: "Simulate in Search",
        type: "boolean",
        defaultValue: false,
        inlineEditable: true,
      },
      {
        name: "is_damage_relevant",
        label: "Is Damage Relevant",
        type: "boolean",
        defaultValue: false,
        inlineEditable: true,
      },
      {
        name: "damage_relevance_reason",
        label: "Damage Relevance Reason",
        type: "text",
        inlineEditable: true,
      },
      ...timestampFields(),
    ],
  },
  {
    name: "realm",
    label: "Realms",
    description:
      "Lookup table for awakener and manifestation realm requirements.",
    displayColumn: "name",
    softDelete: true,
    order: 1.5,
    fields: [
      {
        name: "id",
        label: "ID",
        type: "id",
        autoGenerated: true,
        formHidden: true,
      },
      { name: "name", label: "Name", type: "text", required: true },
      {
        name: "replace",
        label: "Replace",
        type: "foreignKey",
        foreignKey: { table: "realm", displayColumn: "name" },
      },
      ...timestampFields(),
    ],
  },
  {
    name: "realm_tag_manifestation",
    label: "Realm Manifestations",
    description: "Realm–tag pairings with graded values.",
    displayColumn: "id",
    softDelete: true,
    order: 1.6,
    fields: [
      {
        name: "id",
        label: "ID",
        type: "id",
        autoGenerated: true,
        formHidden: true,
      },
      {
        name: "realm_id",
        label: "Realm",
        type: "foreignKey",
        required: true,
        foreignKey: { table: "realm", displayColumn: "name" },
      },
      {
        name: "required_realm_mode",
        label: "Realm Match Mode",
        type: "enum",
        enumName: "realm_match_mode",
        defaultValue: "present",
        inlineEditable: true,
      },
      {
        name: "tag_id",
        label: "Tag",
        type: "foreignKey",
        required: true,
        foreignKey: { table: "tag", displayColumn: "tag_name" },
      },
      {
        name: "trigger_condition",
        label: "Trigger Condition",
        type: "foreignKey",
        foreignKey: { table: "tag", displayColumn: "tag_name" },
      },
      {
        name: "value_scalar",
        label: "Value Scalar",
        type: "number",
        inlineEditable: true,
      },
      {
        name: "dependency_stat",
        label: "Dependency Stat",
        type: "enum",
        enumName: "all_stats",
        inlineEditable: true,
      },
      {
        name: "dependency_rate",
        label: "Dependency Rate",
        type: "number",
        inlineEditable: true,
      },
      {
        name: "dependency_rate_stat",
        label: "Dependency Rate Stat",
        type: "enum",
        enumName: "all_stats",
        inlineEditable: true,
      },
      {
        name: "pure_bonus_target",
        label: "Pure Bonus Target",
        type: "enum",
        enumName: "pure_bonus_target",
        defaultValue: "none",
        inlineEditable: true,
      },
      {
        name: "is_accumulating",
        label: "Is Accumulating",
        type: "boolean",
        defaultValue: false,
        inlineEditable: true,
      },
      {
        name: "is_permanent",
        label: "Is Permanent",
        type: "boolean",
        defaultValue: false,
        inlineEditable: true,
      },
      { name: "metadata", label: "Metadata", type: "textarea" },
      ...timestampFields(),
    ],
  },
  {
    name: "relic",
    label: "Relics",
    description: "Relic catalog with Gold/Cursed variant tiers.",
    displayColumn: "name",
    softDelete: true,
    order: 1.7,
    fields: [
      {
        name: "id",
        label: "ID",
        type: "id",
        autoGenerated: true,
        formHidden: true,
      },
      { name: "name", label: "Name", type: "text", required: true },
      { name: "tier", label: "Tier", type: "text", required: true },
      {
        name: "required_realm",
        label: "Required Realm",
        type: "foreignKey",
        foreignKey: { table: "realm", displayColumn: "name" },
        inlineEditable: true,
      },
      {
        name: "is_damage",
        label: "Is Damage",
        type: "boolean",
        defaultValue: true,
        inlineEditable: true,
      },
      ...timestampFields(),
    ],
  },
  {
    name: "relic_tag_manifestation",
    label: "Relic Manifestations",
    description: "Relic–tag pairings with values.",
    displayColumn: "id",
    softDelete: true,
    order: 1.8,
    fields: [
      {
        name: "id",
        label: "ID",
        type: "id",
        autoGenerated: true,
        formHidden: true,
      },
      {
        name: "relic_id",
        label: "Relic",
        type: "foreignKey",
        required: true,
        foreignKey: { table: "relic", displayColumn: "name" },
      },
      {
        name: "tag_id",
        label: "Tag",
        type: "foreignKey",
        required: true,
        foreignKey: { table: "tag", displayColumn: "tag_name" },
      },
      {
        name: "trigger_condition",
        label: "Trigger Condition",
        type: "foreignKey",
        foreignKey: { table: "tag", displayColumn: "tag_name" },
      },
      {
        name: "value_scalar",
        label: "Value Scalar",
        type: "number",
        inlineEditable: true,
      },
      {
        name: "target_type",
        label: "Target Type",
        type: "enum",
        enumName: "target_type",
        inlineEditable: true,
      },
      {
        name: "kind",
        label: "Kind",
        type: "enum",
        enumName: "relic_arg_kind",
        defaultValue: "fixed",
        required: true,
        inlineEditable: true,
      },
      {
        name: "base_formula",
        label: "Base Formula",
        type: "enum",
        enumName: "relic_base_formula",
        inlineEditable: true,
      },
      {
        name: "dependency_stat",
        label: "Dependency Stat",
        type: "enum",
        enumName: "all_stats",
        inlineEditable: true,
      },
      {
        name: "is_accumulating",
        label: "Is Accumulating",
        type: "boolean",
        defaultValue: false,
        inlineEditable: true,
      },
      {
        name: "is_permanent",
        label: "Is Permanent",
        type: "boolean",
        defaultValue: false,
        inlineEditable: true,
      },
      { name: "metadata", label: "Metadata", type: "textarea" },
      ...timestampFields(),
    ],
  },
  {
    name: "awakener",
    label: "Awakeners",
    description: "Character stats and realm assignments.",
    displayColumn: "name",
    softDelete: true,
    order: 2,
    fields: [
      {
        name: "id",
        label: "ID",
        type: "id",
        autoGenerated: true,
        formHidden: true,
      },
      { name: "name", label: "Name", type: "text", required: true },
      {
        name: "enlightenment",
        label: "Enlightenment",
        type: "number",
        inlineEditable: true,
      },
      {
        name: "realm",
        label: "Realm",
        type: "foreignKey",
        foreignKey: { table: "realm", displayColumn: "name" },
      },
      { name: "con", label: "CON", type: "number" },
      { name: "atk", label: "ATK", type: "number" },
      { name: "def", label: "DEF", type: "number" },
      { name: "keyflare_regen", label: "Keyflare Regen", type: "number" },
      { name: "damage_amp", label: "Damage Amp", type: "number" },
      { name: "crit_rate", label: "Crit Rate", type: "number" },
      { name: "crit_dmg", label: "Crit Damage", type: "number" },
      { name: "realm_mastery", label: "Realm Mastery", type: "number" },
      {
        name: "base_aliemus",
        label: "Base Aliemus",
        type: "number",
        inlineEditable: true,
      },
      { name: "aliemus_regen", label: "Aliemus Regen", type: "number" },
      { name: "sigil_yield", label: "Sigil Yield", type: "number" },
      { name: "death_resist", label: "Base Death Resist", type: "number" },
      {
        name: "notes",
        label: "Kit Notes",
        type: "textarea",
        listHidden: true,
        formWidth: "full",
      },
      ...timestampFields(),
    ],
  },
  {
    name: "desire",
    label: "Desires",
    description: "Team desire definitions for path planning.",
    displayColumn: "name",
    softDelete: true,
    order: 3,
    fields: [
      {
        name: "id",
        label: "ID",
        type: "id",
        autoGenerated: true,
        formHidden: true,
      },
      { name: "name", label: "Name", type: "text", required: true },
      { name: "description", label: "Description", type: "textarea" },
      {
        name: "desire_type",
        label: "Desire Type",
        type: "enum",
        enumName: "desire_type",
        inlineEditable: true,
      },
      {
        name: "anchored_awakener_count",
        label: "Anchored Awakeners",
        type: "number",
        formHidden: true,
        computed: true,
      },
      ...timestampFields(),
    ],
    childTables: [
      {
        table: "desire_anchored_awakener",
        label: "Anchored Awakeners",
        parentKeyColumn: "id",
        foreignKeyColumn: "desire_id",
        fields: [
          {
            name: "awakener_id",
            label: "Awakener",
            type: "foreignKey",
            required: true,
            foreignKey: { table: "awakener", displayColumn: "name" },
          },
          {
            name: "is_damage_dealer",
            label: "Damage Dealer",
            type: "boolean",
            defaultValue: false,
          },
        ],
      },
    ],
  },
  {
    name: "desire_template",
    label: "Desire Templates",
    description:
      "Team loadout presets linked to a desire (posse, awakeners, wheels, covenants).",
    displayColumn: "id",
    softDelete: true,
    order: 4,
    fields: [
      {
        name: "id",
        label: "ID",
        type: "id",
        autoGenerated: true,
        formHidden: true,
      },
      {
        name: "desire_id",
        label: "Desire",
        type: "foreignKey",
        required: true,
        foreignKey: { table: "desire", displayColumn: "name" },
      },
      {
        name: "posse_id",
        label: "Posse",
        type: "foreignKey",
        foreignKey: { table: "posse", displayColumn: "name" },
      },
      ...([1, 2, 3, 4] as const).flatMap((slot) => [
        {
          name: `slot${slot}_awakener_id`,
          label: `Slot ${slot} Awakener`,
          type: "foreignKey" as const,
          listHidden: true,
          foreignKey: { table: "awakener" as const, displayColumn: "name" },
        },
        {
          name: `slot${slot}_wheel1_id`,
          label: `Slot ${slot} Wheel 1`,
          type: "foreignKey" as const,
          listHidden: true,
          foreignKey: { table: "wheel" as const, displayColumn: "name" },
        },
        {
          name: `slot${slot}_wheel2_id`,
          label: `Slot ${slot} Wheel 2`,
          type: "foreignKey" as const,
          listHidden: true,
          foreignKey: { table: "wheel" as const, displayColumn: "name" },
        },
        {
          name: `slot${slot}_covenant_id`,
          label: `Slot ${slot} Covenant`,
          type: "foreignKey" as const,
          listHidden: true,
          foreignKey: { table: "covenant" as const, displayColumn: "name" },
        },
        {
          name: `slot${slot}_covenant_stat_set_id`,
          label: `Slot ${slot} Covenant Stat Set`,
          type: "foreignKey" as const,
          listHidden: true,
          foreignKey: {
            table: "covenant_stat_set" as const,
            displayColumn: "stat",
          },
        },
      ]),
      ...timestampFields(),
    ],
  },
  {
    name: "awakener_tag_manifestation",
    label: "Awakener Manifestations",
    description: "Awakener–tag pairings with graded values and requirements.",
    displayColumn: "id",
    softDelete: true,
    order: 4,
    defaultListSort: { field: "id", direction: "desc" },
    fields: [
      {
        name: "id",
        label: "ID",
        type: "id",
        autoGenerated: true,
        formHidden: true,
      },
      {
        name: "awakener_id",
        label: "Awakener",
        type: "foreignKey",
        required: true,
        foreignKey: { table: "awakener", displayColumn: "name" },
      },
      {
        name: "tag_id",
        label: "Tag",
        type: "foreignKey",
        required: true,
        foreignKey: { table: "tag", displayColumn: "tag_name" },
      },
      {
        name: "trigger_condition",
        label: "Trigger Condition",
        type: "foreignKey",
        foreignKey: { table: "tag", displayColumn: "tag_name" },
      },
      {
        name: "override_count",
        label: "Overrides",
        type: "number",
        formHidden: true,
        computed: true,
      },
      { name: "metadata", label: "Metadata", type: "textarea" },
      {
        name: "replaces_manifestation_id",
        label: "Replaces Manifestation",
        type: "foreignKey",
        foreignKey: {
          table: "awakener_tag_manifestation",
          displayColumn: "metadata",
          filterBy: { column: "awakener_id", formField: "awakener_id" },
        },
      },
      {
        name: "dependency_stat",
        label: "Dependency Stat",
        type: "enum",
        enumName: "all_stats",
        inlineEditable: true,
      },
      {
        name: "value_scalar",
        label: "Value Scalar",
        type: "number",
        inlineEditable: true,
      },
      {
        name: "instance_count",
        label: "Instances",
        type: "number",
        required: true,
        defaultValue: 1,
        inlineEditable: true,
      },
      {
        name: "base_copies",
        label: "Base Copies",
        type: "number",
        required: true,
        defaultValue: 1,
        inlineEditable: true,
      },
      {
        name: "copy_provider_group_id",
        label: "Copy Provider Group",
        type: "foreignKey",
        foreignKey: {
          table: "copy_provider_group",
          displayColumn: "name",
        },
        inlineEditable: true,
      },
      {
        name: "is_accumulating",
        label: "Is Accumulating",
        type: "boolean",
        defaultValue: false,
        inlineEditable: true,
      },
      {
        name: "is_permanent",
        label: "Is Permanent",
        type: "boolean",
        defaultValue: false,
        inlineEditable: true,
      },
      {
        name: "verified",
        label: "Verified",
        type: "boolean",
        defaultValue: true,
        inlineEditable: true,
      },
      {
        name: "required_enlightenment",
        label: "Required Enlightenment",
        type: "number",
        defaultValue: 0,
        inlineEditable: true,
        numberSelectOptions: AWAKENER_ENLIGHTENMENT_OPTIONS,
      },
      {
        name: "required_realm",
        label: "Required Realm",
        type: "foreignKey",
        foreignKey: { table: "realm", displayColumn: "name" },
        inlineEditable: true,
      },
      {
        name: "source_type",
        label: "Source Type",
        type: "enum",
        enumName: "source_type",
        inlineEditable: true,
      },
      {
        name: "target_type",
        label: "Target Type",
        type: "enum",
        enumName: "target_type",
        inlineEditable: true,
      },
      {
        name: "buff_target_type_restriction",
        label: "Buff Restriction",
        type: "enum",
        enumName: "source_type",
        inlineEditable: true,
      },
      ...timestampFields(),
    ],
    childTables: [
      {
        table: "awakener_local_manifestation_interaction",
        label: "Local Interactions",
        parentKeyColumn: "id",
        foreignKeyColumn: "manifestation_id",
        fields: [
          {
            name: "mode",
            label: "Mode",
            type: "enum",
            enumName: "awakener_local_interaction_mode",
            defaultValue: "unique_scaling",
            required: true,
          },
          {
            name: "modifier_tag_id",
            label: "Modifier Tag",
            type: "foreignKey",
            foreignKey: { table: "tag", displayColumn: "tag_name" },
          },
          {
            name: "target_tag_id",
            label: "Target Tag",
            type: "foreignKey",
            foreignKey: { table: "tag", displayColumn: "tag_name" },
          },
          {
            name: "layer",
            label: "Layer",
            type: "enum",
            enumName: "layer",
          },
          {
            name: "math_operation",
            label: "Math Operation",
            type: "enum",
            enumName: "operation_type",
            defaultValue: "multiply_one_plus",
          },
          {
            name: "value_scalar",
            label: "Value Scalar",
            type: "number",
            defaultValue: 1,
            required: true,
          },
          {
            name: "target_type",
            label: "Target Type",
            type: "enum",
            enumName: "target_type",
            defaultValue: "aoe",
            required: true,
          },
          {
            name: "dependency_stat",
            label: "Dependency Stat",
            type: "enum",
            enumName: "all_stats",
          },
          { name: "is_disabled", label: "Disabled", type: "boolean" },
        ],
      },
    ],
  },
  {
    name: "tag_default_interaction",
    label: "Tag Interactions",
    description: "Default interaction rules between modifier and target tags.",
    displayColumn: "id",
    softDelete: true,
    order: 5,
    uniqueConstraints: [
      {
        fields: [
          "modifier_tag_id",
          "target_tag_id",
          "buff_target_type_restriction",
        ],
        ignoreDeleted: true,
        message:
          "An active interaction with this modifier tag, target tag, and buff restriction already exists.",
      },
    ],
    fields: [
      {
        name: "id",
        label: "ID",
        type: "id",
        autoGenerated: true,
        formHidden: true,
      },
      {
        name: "modifier_tag_id",
        label: "Modifier Tag",
        type: "foreignKey",
        foreignKey: { table: "tag", displayColumn: "tag_name" },
        required: true,
        inlineEditable: true,
      },
      {
        name: "target_tag_id",
        label: "Target Tag",
        type: "foreignKey",
        foreignKey: { table: "tag", displayColumn: "tag_name" },
        required: true,
        inlineEditable: true,
      },
      {
        name: "creates_base",
        label: "Creates Base",
        type: "boolean",
        inlineEditable: true,
        defaultValue: false,
        formWidth: "half",
      },
      {
        name: "amplifies_subject",
        label: "Amplifies Subject",
        type: "boolean",
        inlineEditable: true,
        defaultValue: true,
        formWidth: "half",
      },
      {
        name: "exclusion_suffix",
        label: "Exclude Tag",
        type: "foreignKey",
        foreignKey: { table: "tag", displayColumn: "tag_name" },
        inlineEditable: true,
      },
      {
        name: "math_operation",
        label: "Math Operation",
        type: "enum",
        enumName: "operation_type",
        required: true,
        inlineEditable: true,
      },
      {
        name: "default_factor",
        label: "Default Factor",
        type: "number",
        inlineEditable: true,
      },
      {
        name: "buff_target_type_restriction",
        label: "Buff Restriction",
        type: "enum",
        enumName: "source_type",
        inlineEditable: true,
      },
      ...timestampFields(),
    ],
  },
  {
    name: "awakener_local_manifestation_interaction",
    label: "Local Interactions",
    description:
      "Per-manifestation local interactions: unique_scaling (modifier tag or base-stat dep → parent tag) or aftereffect (finished subject→target tag).",
    displayColumn: "id",
    softDelete: true,
    order: 6,
    fields: [
      {
        name: "id",
        label: "ID",
        type: "id",
        autoGenerated: true,
        formHidden: true,
      },
      {
        name: "manifestation_id",
        label: "Manifestation",
        type: "foreignKey",
        foreignKey: {
          table: "awakener_tag_manifestation",
          displayColumn: "id",
          labelKind: "manifestation",
        },
        required: true,
      },
      {
        name: "mode",
        label: "Mode",
        type: "enum",
        enumName: "awakener_local_interaction_mode",
        defaultValue: "unique_scaling",
        required: true,
        inlineEditable: true,
      },
      {
        name: "modifier_tag_id",
        label: "Modifier Tag",
        type: "foreignKey",
        foreignKey: { table: "tag", displayColumn: "tag_name" },
      },
      {
        name: "target_tag_id",
        label: "Target Tag",
        type: "foreignKey",
        foreignKey: { table: "tag", displayColumn: "tag_name" },
      },
      {
        name: "layer",
        label: "Layer",
        type: "enum",
        enumName: "layer",
        inlineEditable: true,
      },
      {
        name: "math_operation",
        label: "Math Operation",
        type: "enum",
        enumName: "operation_type",
        defaultValue: "multiply_one_plus",
        inlineEditable: true,
      },
      {
        name: "value_scalar",
        label: "Value Scalar",
        type: "number",
        defaultValue: 1,
        required: true,
        inlineEditable: true,
      },
      {
        name: "target_type",
        label: "Target Type",
        type: "enum",
        enumName: "target_type",
        defaultValue: "aoe",
        required: true,
        inlineEditable: true,
      },
      {
        name: "dependency_stat",
        label: "Dependency Stat",
        type: "enum",
        enumName: "all_stats",
        inlineEditable: true,
      },
      {
        name: "is_disabled",
        label: "Disabled",
        type: "boolean",
        inlineEditable: true,
      },
      ...timestampFields(),
    ],
  },
  {
    name: "desire_demand",
    label: "Desire Demands",
    description: "Tag demand curves tied to a desire.",
    displayColumn: "id",
    softDelete: true,
    order: 7,
    fields: [
      {
        name: "id",
        label: "ID",
        type: "id",
        autoGenerated: true,
        formHidden: true,
      },
      {
        name: "desire_id",
        label: "Desire",
        type: "foreignKey",
        required: true,
        foreignKey: { table: "desire", displayColumn: "name" },
      },
      {
        name: "tag_id",
        label: "Tag",
        type: "foreignKey",
        required: true,
        foreignKey: { table: "tag", displayColumn: "tag_name" },
      },
      {
        name: "base_priority_weight",
        label: "Base Priority Weight",
        type: "number",
      },
      { name: "target_value", label: "Target Value", type: "number" },
      {
        name: "curve",
        label: "Curve",
        type: "enum",
        enumName: "curve_type",
      },
      { name: "decay_rate", label: "Decay Rate", type: "number" },
      ...timestampFields(),
    ],
  },
  {
    name: "copy_provider_group",
    label: "Copy Provider Groups",
    description:
      "Curated tag sets that add extra copies to ATMs via copy_provider_group_id.",
    displayColumn: "name",
    softDelete: true,
    order: 7,
    uniqueConstraints: [
      {
        fields: ["name"],
        ignoreDeleted: true,
        message: "A copy provider group with this name already exists.",
      },
    ],
    childTables: [
      {
        table: "copy_provider_group_member",
        label: "Members",
        parentKeyColumn: "id",
        foreignKeyColumn: "group_id",
        fields: [
          {
            name: "tag_id",
            label: "Tag",
            type: "foreignKey",
            required: true,
            foreignKey: { table: "tag", displayColumn: "tag_name" },
          },
        ],
      },
    ],
    fields: [
      {
        name: "id",
        label: "ID",
        type: "id",
        autoGenerated: true,
        formHidden: true,
      },
      { name: "name", label: "Name", type: "text", required: true },
      ...timestampFields(),
    ],
  },
  {
    name: "copy_provider_group_member",
    label: "Copy Provider Group Members",
    description: "Tags that belong to a copy provider group.",
    displayColumn: "id",
    softDelete: true,
    order: 7,
    sidebarHidden: true,
    uniqueConstraints: [
      {
        fields: ["group_id", "tag_id"],
        ignoreDeleted: true,
        message: "This tag is already in the group.",
      },
    ],
    fields: [
      {
        name: "id",
        label: "ID",
        type: "id",
        autoGenerated: true,
        formHidden: true,
      },
      {
        name: "group_id",
        label: "Group",
        type: "foreignKey",
        required: true,
        foreignKey: { table: "copy_provider_group", displayColumn: "name" },
      },
      {
        name: "tag_id",
        label: "Tag",
        type: "foreignKey",
        required: true,
        foreignKey: { table: "tag", displayColumn: "tag_name" },
      },
      ...timestampFields(),
    ],
  },
  {
    name: "path",
    label: "Paths",
    description: "Links between an awakener and a desire.",
    displayColumn: "id",
    softDelete: false,
    order: 8,
    fields: [
      {
        name: "id",
        label: "ID",
        type: "id",
        autoGenerated: true,
        formHidden: true,
      },
      {
        name: "awakener_id",
        label: "Awakener",
        type: "foreignKey",
        required: true,
        foreignKey: { table: "awakener", displayColumn: "name" },
      },
      {
        name: "desire_id",
        label: "Desire",
        type: "foreignKey",
        required: true,
        foreignKey: { table: "desire", displayColumn: "name" },
      },
    ],
  },
  {
    name: "desire_anchored_awakener",
    label: "Anchored Awakeners",
    description: "Awakeners anchored to a specific desire.",
    displayColumn: "id",
    softDelete: true,
    order: 9,
    fields: [
      {
        name: "id",
        label: "ID",
        type: "id",
        autoGenerated: true,
        formHidden: true,
      },
      {
        name: "desire_id",
        label: "Desire",
        type: "foreignKey",
        required: true,
        foreignKey: { table: "desire", displayColumn: "name" },
      },
      {
        name: "awakener_id",
        label: "Awakener",
        type: "foreignKey",
        required: true,
        foreignKey: { table: "awakener", displayColumn: "name" },
      },
      {
        name: "is_damage_dealer",
        label: "Damage Dealer",
        type: "boolean",
        defaultValue: false,
      },
      ...timestampFields(),
    ],
  },
  {
    name: "posse",
    label: "Posse",
    description: "Posse definitions for team loadouts.",
    displayColumn: "name",
    softDelete: true,
    order: 10,
    fields: [
      {
        name: "id",
        label: "ID",
        type: "id",
        autoGenerated: true,
        formHidden: true,
      },
      { name: "name", label: "Name", type: "text", required: true },
      ...timestampFields(),
    ],
  },
  {
    name: "posse_tag_manifestation",
    label: "Posse Manifestations",
    description: "Posse–tag pairings with requirements and values.",
    displayColumn: "id",
    softDelete: true,
    order: 11,
    fields: [
      {
        name: "id",
        label: "ID",
        type: "id",
        autoGenerated: true,
        formHidden: true,
      },
      {
        name: "posse_id",
        label: "Posse",
        type: "foreignKey",
        required: true,
        foreignKey: { table: "posse", displayColumn: "name" },
      },
      {
        name: "tag_id",
        label: "Tag",
        type: "foreignKey",
        required: true,
        foreignKey: { table: "tag", displayColumn: "tag_name" },
      },
      {
        name: "group_key",
        label: "Group Key",
        type: "text",
        required: true,
        defaultValue: "1",
        inlineEditable: true,
      },
      {
        name: "dependency_stat",
        label: "Dependency Stat",
        type: "enum",
        enumName: "all_stats",
        inlineEditable: true,
      },
      {
        name: "required_awakener",
        label: "Required Awakener",
        type: "foreignKey",
        foreignKey: { table: "awakener", displayColumn: "name" },
      },
      {
        name: "required_realm",
        label: "Required Realm",
        type: "foreignKey",
        foreignKey: { table: "realm", displayColumn: "name" },
        inlineEditable: true,
      },
      {
        name: "target_type",
        label: "Target Type",
        type: "enum",
        enumName: "target_type",
        inlineEditable: true,
      },
      {
        name: "buff_target_type_restriction",
        label: "Buff Restriction",
        type: "enum",
        enumName: "source_type",
        inlineEditable: true,
      },
      {
        name: "value_scalar",
        label: "Value Scalar",
        type: "number",
        inlineEditable: true,
      },
      {
        name: "is_accumulating",
        label: "Is Accumulating",
        type: "boolean",
        defaultValue: false,
        inlineEditable: true,
      },
      {
        name: "is_permanent",
        label: "Is Permanent",
        type: "boolean",
        defaultValue: false,
        inlineEditable: true,
      },
      { name: "metadata", label: "Metadata", type: "textarea" },
      ...timestampFields(),
    ],
  },
  {
    name: "wheel",
    label: "Wheels",
    description: "Wheel gear with rarity and stat bonuses.",
    displayColumn: "name",
    softDelete: true,
    order: 12,
    fields: [
      {
        name: "id",
        label: "ID",
        type: "id",
        autoGenerated: true,
        formHidden: true,
      },
      { name: "name", label: "Name", type: "text", required: true },
      {
        name: "enlightenment",
        label: "Enlightenment",
        type: "number",
        inlineEditable: true,
      },
      {
        name: "rarity",
        label: "Rarity",
        type: "enum",
        enumName: "rarity",
        inlineEditable: true,
      },
      {
        name: "stat",
        label: "Stat",
        type: "enum",
        enumName: "all_stats",
        inlineEditable: true,
      },
      {
        name: "stat_amount",
        label: "Stat Amount",
        type: "number",
        inlineEditable: true,
      },
      ...timestampFields(),
    ],
  },
  {
    name: "wheel_tag_manifestation",
    label: "Wheel Manifestations",
    description: "Wheel–tag pairings with graded values and requirements.",
    displayColumn: "id",
    softDelete: true,
    order: 13,
    fields: [
      {
        name: "id",
        label: "ID",
        type: "id",
        autoGenerated: true,
        formHidden: true,
      },
      {
        name: "wheel_id",
        label: "Wheel",
        type: "foreignKey",
        required: true,
        foreignKey: { table: "wheel", displayColumn: "name" },
      },
      {
        name: "tag_id",
        label: "Tag",
        type: "foreignKey",
        required: true,
        foreignKey: { table: "tag", displayColumn: "tag_name" },
      },
      {
        name: "trigger_condition",
        label: "Trigger Condition",
        type: "foreignKey",
        foreignKey: { table: "tag", displayColumn: "tag_name" },
      },
      {
        name: "dependency_stat",
        label: "Dependency Stat",
        type: "enum",
        enumName: "all_stats",
        inlineEditable: true,
      },
      {
        name: "buff_target_type_restriction",
        label: "Buff Restriction",
        type: "enum",
        enumName: "source_type",
        inlineEditable: true,
      },
      {
        name: "required_realm",
        label: "Required Realm",
        type: "foreignKey",
        foreignKey: { table: "realm", displayColumn: "name" },
        inlineEditable: true,
      },
      {
        name: "target_type",
        label: "Target Type",
        type: "enum",
        enumName: "target_type",
        inlineEditable: true,
      },
      {
        name: "value_scalar",
        label: "Value Scalar",
        type: "number",
        inlineEditable: true,
      },
      {
        name: "is_accumulating",
        label: "Is Accumulating",
        type: "boolean",
        defaultValue: false,
        inlineEditable: true,
      },
      {
        name: "is_permanent",
        label: "Is Permanent",
        type: "boolean",
        defaultValue: false,
        inlineEditable: true,
      },
      { name: "metadata", label: "Metadata", type: "textarea" },
      ...timestampFields(),
    ],
  },
  {
    name: "covenant",
    label: "Covenants",
    description: "Covenant gear with stat bonuses.",
    displayColumn: "name",
    softDelete: true,
    order: 14,
    fields: [
      {
        name: "id",
        label: "ID",
        type: "id",
        autoGenerated: true,
        formHidden: true,
      },
      { name: "name", label: "Name", type: "text", required: true },
      {
        name: "stat",
        label: "Stat",
        type: "enum",
        enumName: "all_stats",
        inlineEditable: true,
      },
      {
        name: "stat_amount",
        label: "Stat Amount",
        type: "number",
        inlineEditable: true,
      },
      {
        name: "team_unique",
        label: "Team Unique",
        type: "boolean",
        defaultValue: false,
        inlineEditable: true,
      },
      ...timestampFields(),
    ],
  },
  {
    name: "covenant_stat_set",
    label: "Covenant Stat Sets",
    description: "Reusable covenant flat-stat bonuses.",
    displayColumn: "stat",
    softDelete: true,
    order: 14.5,
    fields: [
      {
        name: "id",
        label: "ID",
        type: "id",
        autoGenerated: true,
        formHidden: true,
      },
      {
        name: "stat",
        label: "Stat",
        type: "enum",
        enumName: "all_stats",
        inlineEditable: true,
      },
      {
        name: "stat_amount",
        label: "Stat Amount",
        type: "number",
        inlineEditable: true,
      },
      ...timestampFields(),
    ],
  },
  {
    name: "covenant_tag_manifestation",
    label: "Covenant Manifestations",
    description: "Covenant–tag pairings with graded values and requirements.",
    displayColumn: "id",
    softDelete: true,
    order: 15,
    fields: [
      {
        name: "id",
        label: "ID",
        type: "id",
        autoGenerated: true,
        formHidden: true,
      },
      {
        name: "covenant_id",
        label: "Covenant",
        type: "foreignKey",
        required: true,
        foreignKey: { table: "covenant", displayColumn: "name" },
      },
      {
        name: "tag_id",
        label: "Tag",
        type: "foreignKey",
        required: true,
        foreignKey: { table: "tag", displayColumn: "tag_name" },
      },
      {
        name: "trigger_condition",
        label: "Trigger Condition",
        type: "foreignKey",
        foreignKey: { table: "tag", displayColumn: "tag_name" },
      },
      {
        name: "dependency_stat",
        label: "Dependency Stat",
        type: "enum",
        enumName: "all_stats",
        inlineEditable: true,
      },
      {
        name: "replaces_manifestation_id",
        label: "Replaces Manifestation",
        type: "foreignKey",
        foreignKey: {
          table: "covenant_tag_manifestation",
          displayColumn: "metadata",
          filterBy: { column: "covenant_id", formField: "covenant_id" },
        },
      },
      {
        name: "is_accumulating",
        label: "Is Accumulating",
        type: "boolean",
        defaultValue: false,
        inlineEditable: true,
      },
      {
        name: "is_permanent",
        label: "Is Permanent",
        type: "boolean",
        defaultValue: false,
        inlineEditable: true,
      },
      {
        name: "required_realm1",
        label: "Required Realm 1",
        type: "foreignKey",
        foreignKey: { table: "realm", displayColumn: "name" },
        inlineEditable: true,
        formWidth: "half",
      },
      {
        name: "required_realm2",
        label: "Required Realm 2",
        type: "foreignKey",
        foreignKey: { table: "realm", displayColumn: "name" },
        inlineEditable: true,
        formWidth: "half",
      },
      {
        name: "target_type",
        label: "Target Type",
        type: "enum",
        enumName: "target_type",
        inlineEditable: true,
      },
      {
        name: "buff_target_type_restriction",
        label: "Buff Restriction",
        type: "enum",
        enumName: "source_type",
        inlineEditable: true,
      },
      {
        name: "value_scalar",
        label: "Value Scalar",
        type: "number",
        inlineEditable: true,
      },
      { name: "metadata", label: "Metadata", type: "textarea" },
      ...timestampFields(),
    ],
  },
];

export const TABLE_CONFIG_MAP = Object.fromEntries(
  TABLE_CONFIGS.map((config) => [config.name, config]),
) as Record<TableName, TableConfig>;

export type SidebarNavGroup = {
  label: string;
  tables: TableName[];
};

export const SIDEBAR_NAV_GROUPS: SidebarNavGroup[] = [
  { label: "Tags", tables: ["tag", "tag_default_interaction"] },
  { label: "Realms", tables: ["realm", "realm_tag_manifestation"] },
  {
    label: "Awakeners",
    tables: [
      "awakener",
      "awakener_tag_manifestation",
      "awakener_local_manifestation_interaction",
      "copy_provider_group",
    ],
  },
  { label: "Wheels", tables: ["wheel", "wheel_tag_manifestation"] },
  {
    label: "Covenants",
    tables: ["covenant", "covenant_stat_set", "covenant_tag_manifestation"],
  },
  { label: "Posse", tables: ["posse", "posse_tag_manifestation"] },
  { label: "Relics", tables: ["relic", "relic_tag_manifestation"] },
  {
    label: "Desires",
    tables: [
      "path",
      "desire",
      "desire_template",
      "desire_demand",
      "desire_anchored_awakener",
    ],
  },
];

export function isValidTableName(name: string): name is TableName {
  return name in TABLE_CONFIG_MAP;
}

export function getListFields(config: TableConfig) {
  return config.fields.filter((field) => !field.listHidden);
}

export function getDefaultListSort(config: TableConfig): ListSortState {
  if (config.defaultListSort) {
    return {
      field: config.defaultListSort.field,
      direction: config.defaultListSort.direction,
    };
  }
  return { field: null, direction: "asc" };
}

export function getFormFields(config: TableConfig) {
  return config.fields.filter((field) => !field.formHidden);
}

export function getTableTitle(tableName: TableName) {
  return TABLE_CONFIG_MAP[tableName].label;
}

export function getFieldLabel(fieldName: string) {
  return formatLabel(fieldName);
}
