...
How to refresh icons (wheel / posse / covenant / relic) and the pinned SKeyDB art commit used by Search, Path Carver, and the Relic Picker.

Awakener portraits resolve by name slug against the same pin; they do **not** use the name→asset JSON maps. Wheel, posse, covenant, and relic icons do use them.

## When to run this

- You added or renamed a **wheel**, **posse**, **covenant**, or **relic** in Mother Tree and its icon is missing
...
3. Commit and deploy:
   ...
   - `src/lib/assets/maps/wheel-by-name.json`, `covenant-by-name.json`, `posse-by-name.json`, `relic-by-name.json`
   - `sample-data/skeydb-asset-names/*`

## What `sync:skeydb-assets` does

1. **Slim dump** — `npm run db:dump-skeydb-assets` writes alive `name` values for wheel / covenant / posse / relic to `sample-data/skeydb-asset-names/`
2. **Bump pin** — sets `SKEYDB_COMMIT` to the latest `main` tip of SKeyDB (unless you pass a SHA)
3. **Regenerate maps** — `npm run generate:skeydb-assets` joins those names to the SKeyDB `wheels` / `covenants` / `posses` / `relics` catalogs at the pin
