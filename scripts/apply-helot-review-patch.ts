/**
 * Apply Helot Kit Reader review patch:
 * - Update Shattering Strike (id 661) with 10x STR Up unique_scaling local
 * - Insert new base rows:
 *     - lasting-loathe-create-strike (Support.Create.Command Card.Strike, value 2, target aoe)
 *     - shattering-strike-hand-scaling (Support.Base Damage.Strike, value 1.35, target self)
 *     - sf-embryo-crit-dmg (Support.Crit Damage, value 1.0, target self)
 * - Insert E1 replace rows (baseCopies: 2 for Lasting Loathe records):
 *     - lasting-loathe-e1-self-damage (replaces lasting-loathe-self-damage id 659)
 *     - lasting-loathe-e1-damage (replaces lasting-loathe-damage id 660)
 *     - lasting-loathe-e1-create-strike (replaces lasting-loathe-create-strike)
 */
...
const SHATTERING_STRIKE_DAMAGE_ID = 661;
const LASTING_LOATHE_SELF_DAMAGE_ID = 659;
const LASTING_LOATHE_DAMAGE_ID = 660;
...
async function main() {
  ...
  await assertLivePending(supabase, SHATTERING_STRIKE_DAMAGE_ID);
  await assertLivePending(supabase, LASTING_LOATHE_SELF_DAMAGE_ID);
  await assertLivePending(supabase, LASTING_LOATHE_DAMAGE_ID);

  // 1. Add 10x STR Up local unique_scaling to Shattering Strike (id 661)
  const shatteringLocal: KitAtmProposal["locals"][number] = {
    mode: "unique_scaling",
    modifierTagName: "Support.STR Up",
    targetTagName: null,
    dependencyStat: null,
  ...
  const localRow = resolveLocalRow(shatteringLocal, SHATTERING_STRIKE_DAMAGE_ID, maps);
  const { error: localErr } = await supabase
    .from("awakener_local_manifestation_interaction")
    .insert([localRow] as never);
  if (localErr) throw new Error(localErr.message);
  updated.push({
    id: SHATTERING_STRIKE_DAMAGE_ID,
    action: "added 10x STR Up unique_scaling local",
  });
  ...
  const clientKeyToId = new Map<string, number>([
    ["shattering-strike-damage", SHATTERING_STRIKE_DAMAGE_ID],
    ["lasting-loathe-self-damage", LASTING_LOATHE_SELF_DAMAGE_ID],
    ["lasting-loathe-damage", LASTING_LOATHE_DAMAGE_ID],
  ]);

  // 2. Insert new base rows (not yet inserted)
  const newBaseKeys = [
    "lasting-loathe-create-strike",
    "shattering-strike-hand-scaling",
    "sf-embryo-crit-dmg",
  ];

  for (const key of newBaseKeys) {
    const proposal = full.proposals.find((p) => p.clientKey === key);
    if (!proposal) throw new Error(`Missing proposal for ${key}`);
  ...
  // 3. Insert E1 replace rows
  const e1ReplaceKeys = [
    "lasting-loathe-e1-self-damage",
    "lasting-loathe-e1-damage",
    "lasting-loathe-e1-create-strike",
  ];

  for (const key of e1ReplaceKeys) {
    const proposal = full.proposals.find((p) => p.clientKey === key);
    if (!proposal) throw new Error(`Missing proposal for ${key}`);
    const replacesId =
    ...
    if (proposal.replacesClientKey != null && replacesId == null) {
      throw new Error(`Missing replace target for ${proposal.clientKey} (replacesClientKey=${proposal.replacesClientKey})`);
    }
    const row = await insertProposal(
      supabase,
      proposal,
  ...
  console.log(
    JSON.stringify({ updated, insertedCount: inserted.length, inserted }, null, 2),
  );
}
